...
function FilterByTagDlg(props: {
  ...
  const [flagInclude] = useState(filterBy?.flagInclude)
  const [flagExclude, setFlagExclude] = useState(filterBy?.flagExclude)
  const [tag] = useState(filterBy?.tagFilter?.tag || '')
  const [tagValue] = useState(filterBy?.tagFilter?.value || '')
  const [readName, setReadName] = useState(filterBy?.readName || '')
  const [readLength, setReadLength] = useState(filterBy?.readLength || '')
  const [onlySplit, setOnlySplit] = useState(filterBy?.onlySplit || false)
  const [excludeSplit, setExcludeSplit] = useState(filterBy?.excludeSplit || false)
      ...
      <DialogContent>
        ...
        <div className={classes.root}>
          ...
          <Paper className={classes.paper} variant="outlined">
            ...
            <Paper className={classes.paper} variant="outlined">
            <Typography>Filter by read length</Typography>
            <TextField
              className={classes.field}
              value={readLength}
              onChange={event => {
                setReadLength(parseInt(event.target.value))
              }}
              placeholder="Enter read length"
              inputProps={{
                'data-testid': 'color-tag-readlen-input',
              }}
              data-testid="color-tag-readlen"
            />
          </Paper>
          <Paper className={classes.paper} variant="outlined">
            <FormControlLabel
                ...
                <Checkbox
                    checked={!!onlySplit}
                    onChange={() => setOnlySplit(val => !val)}
                />
                }
                label="Display ONLY translocation reads"
            />
            <FormControlLabel
                control={
                <Checkbox
                    checked={!!excludeSplit}
                    onChange={() => setExcludeSplit(val => !val)}
                />
                }
                label="Display NOT translocation reads"
            />
          </Paper>
          <Button
            variant="contained"
